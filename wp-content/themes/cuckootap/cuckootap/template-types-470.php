...
*
*
** Template Name: Portfolio 2 Columns by Work Type
*/
	...
	<?php
		...
		if ( !post_password_required() ) : 
			...
			<article id="portfolio" class="clerfix-isotope screen-large-portfolio">
				<?php
				/* the same query need to contunue */
				if ( have_posts() ):
					while ( have_posts() ) : the_post();
						...
						<a class="<?php echo $cats; ?> portfolio-item-470 " href="<?php echo get_permalink(); ?>" title="<?php echo cuckoo_max_trim(the_title('', '', false), 70); ?>">
							<div class="fadeItems">
								<?php if( $video && !has_post_thumbnail()  ) : ?>
									<img class="item-group-0 fade-item-470" src="<?php echo getVideoThumbnail($video, "thumbnail_medium"); ?>" alt="video" />
								<?php elseif ( has_post_thumbnail() ) : ?>
									<?php the_post_thumbnail( 'thumb-470', array( 'title' => '', 'class' => 'item-group-0 fade-item-470' ) ); ?>
								<?php elseif( $gallerythumb && !has_post_thumbnail() ): ?>
									<?php for( $i = 1; $i <= 10; $i++ ){
										...
										if( $url != null ) : ?>
											<img class="item-group-<?php echo $i; ?> fade-item-470"  alt="<?php echo $text; ?>" src="<?php echo cuckoo_get_attachment_all_size($url , 'thumb-470'); ?>" />
										<?php endif;	
									} ?>
								<?php else : ?>
